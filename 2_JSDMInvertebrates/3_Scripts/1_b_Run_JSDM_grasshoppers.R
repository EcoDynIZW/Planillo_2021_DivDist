...
Output_wd <- file.path(WorkDir, "5_Results/InvertebrateModels_Grasshoppers_poisson/")


## Species data

# grasshoppers data
grasshop <- read.csv(paste0(Species_wd, "/grasshoppers_42plots_3sites.csv"))
str(grasshop)
head(grasshop)
ncol(grasshop)
# [1] 18 Species remaining
Y.grasshop <- as.matrix(grasshop)


## Environmental data
env100m.tmp <- read.csv(paste0(Env_wd, "/bibsplots_allenvir_100m.csv"))
env500m.tmp <- read.csv(paste0(Env_wd, "/bibsplots_allenvir_500m.csv"))
...
XFormula.grasshop = ~ imperv.100m + open.green.500m
...
m <- Hmsc(Y = Y.grasshop, XData = Covariates, XFormula = XFormula.grasshop,
...
m <- sampleMcmc(m, thin = thin, samples = samples, transient = transient,
                ...
                nParallel = 3)
#Save the model
filename = paste0(Output_wd, "/model_pois_grasshop_abu_3sites.rds")
saveRDS(m,file=filename)



#########################################
# Grasshoppers model: model check
#########################################
...
# Graphical output
png(paste0(Output_wd, "/grasshoppers_model_convergence_hist.png"), width = 800, height = 1000,
    pointsize = 20)
par(mfrow=c(3,2))
hist(ess.beta, main = "ess(beta)_grasshoppers")
hist(ess.gamma, main = "ess(gamma)_grasshoppers")
hist(ess.omega, main = "ess(omega)_grasshoppers")
hist(gd.beta, main = "psrf(beta)_grasshoppers")
hist(gd.gamma, main = "psrf(gamma)_grasshoppers")
hist(gd.omega, main = "psrf(omega)_grasshoppers")
dev.off()
...
MCMCtrace(mpost$Beta, 
          pdf = TRUE, 
          open_pdf = FALSE,
          filename = "grasshoppers_MCMCtrace_beta",
          wd = Output_wd)
MCMCtrace(mpost$Gamma, 
          pdf = TRUE, 
          open_pdf = FALSE,
          filename = "grasshoppers_MCMCtrace_gamma",
          wd = Output_wd)
MCMCtrace(mpost$Omega[[1]], 
          pdf = TRUE, 
          open_pdf = FALSE,
          filename = "grasshoppers_MCMCtrace_omega",
          wd = Output_wd)
...
write.csv(Beta.results, paste0(Output_wd, "/grasshoppers_beta_coeffients.csv"), 
          row.names = FALSE)
# Default beta plot in Hmsc package
postBeta <- getPostEstimate(m, parName = "Beta")

# Print a plot for each predictor
plot.cov.betas(m, modelType = "grasshoppers") 

# Gamma values
Gamma.results <- as.data.frame(MCMCsummary(mpost$Gamma))
write.csv(Gamma.results, paste0(Output_wd, "/grasshoppers_gamma_coeffients.csv"), 
          row.names = FALSE)
...
png(paste0(Output_wd, "/grasshoppers_default_omegaplot.png"))
...
VP <- computeVariancePartitioning(m, group = c(1,1,2), groupnames = c("urban", "nature"))

# plot var part
png(paste0(Output_wd, "/grasshoppers_default_VP.png"), 
    width = 800)
...
#########################################
# Grasshoppers model: predictions
#########################################
...
Output_wd <- file.path(WorkDir, "5_Results/InvertebrateModels_Grasshoppers_poisson/Predictions")


m$XFormula
# ~imperv.100m + open.green.500m 
...
grid.tmp500 <- read.csv(file.path(Pred_wd, "/transects_allenvir_500m.csv"))
...
m$Y
write.csv(predY2, paste0(Output_wd, "/grasshoppers_predictions_pois_birdtransects.csv"))
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
# Predicting **grasshopper** community in Berlin #
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#


# We are going to use **100 m resolution**
##details in carabid script
...
grid.100mres.grasshop <- grid.topredict.100mres %>%
  dplyr::select(x = x_100m, y = y_100m, 
                imperv.100m = impervious_surface_100m,
                open.green.500m = open_green_500m)
head(grid.100mres.grasshop)


# Remove NAs to improve code efficiency
grid2.100mres <- grid.100mres.grasshop[complete.cases(grid.100mres.grasshop), ]
head(grid2.100mres)
str(grid2.100mres)
...
grasshop.pred.abu <- rowSums(res.df[,-1], na.rm=TRUE)
# Richness per site. We decide to use an abundance higher than 0.5 as presence
grasshop.pred.rich <- rowSums(res.df[,-1] > 0.5, na.rm=TRUE)
# Add coordinates and put all the data together
grasshop.pred <- cbind(x = grid2.100mres$x, y = grid2.100mres$y, res.df, 
                      grasshop.pred.abu = grasshop.pred.abu, 
                      grasshop.pred.rich = grasshop.pred.rich)
# Save data
write.csv(grasshop.pred , paste0(Output_wd, "/Predict_Berlin_100m_grasshoppers_3sites.csv"), 
          row.names = F)
