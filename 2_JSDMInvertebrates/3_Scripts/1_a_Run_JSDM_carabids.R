...
Output_wd <- file.path(WorkDir, "5_Results/InvertebrateModels_Carabids_poisson/")


## Species data

# carabids data
carabids <- read.csv(paste0(Species_wd, "/carabids_42plots_3sites.csv"))
str(carabids)
head(carabids)
ncol(carabids)
# [1] 73 Species remaining
Y.carabids <- as.matrix(carabids)
...
XFormula.carabids = ~ imperv.100m + dist.water.100m + temp.100m
...
m <- Hmsc(Y = Y.carabids, XData = Covariates, XFormula = XFormula.carabids,
...
m <- sampleMcmc(m, thin = thin, samples = samples, transient = transient,
                ...
                nParallel = 3)
#Save the model
filename = paste0(Output_wd, "/model_pois_carabids_abu_3sites.rds")
saveRDS(m,file=filename)



#########################################
# Carabids model: model check
#########################################
...
# Graphical output
png(paste0(Output_wd, "/carabids_model_convergence_hist.png"), width = 800, height = 1000,
    pointsize = 20)
par(mfrow=c(3,2))
hist(ess.beta, main = "ess(beta)_carabids")
hist(ess.gamma, main = "ess(gamma)_carabids")
hist(ess.omega, main = "ess(omega)_carabids")
hist(gd.beta, main = "psrf(beta)_carabids")
hist(gd.gamma, main = "psrf(gamma)_carabids")
hist(gd.omega, main = "psrf(omega)_carabids")
dev.off()
...
MCMCtrace(mpost$Beta, 
          pdf = TRUE, 
          open_pdf = FALSE,
          filename = "carabids_MCMCtrace_beta",
          wd = Output_wd)
MCMCtrace(mpost$Gamma, 
          pdf = TRUE, 
          open_pdf = FALSE,
          filename = "carabids_MCMCtrace_gamma",
          wd = Output_wd)
MCMCtrace(mpost$Omega[[1]], 
          pdf = TRUE, 
          open_pdf = FALSE,
          filename = "carabids_MCMCtrace_omega",
          wd = Output_wd)
...
write.csv(Beta.results, paste0(Output_wd, "/carabids_beta_coeffients.csv"), 
          row.names = FALSE)
# Default beta plot in Hmsc package
postBeta <- getPostEstimate(m, parName = "Beta")

# Print a plot for each predictor
plot.cov.betas(m, modelType = "carabids") 

# Gamma values
Gamma.results <- as.data.frame(MCMCsummary(mpost$Gamma))
write.csv(Gamma.results, paste0(Output_wd, "/carabids_gamma_coeffients.csv"), 
          row.names = FALSE)
...
png(paste0(Output_wd, "/carabids_default_omegaplot.png"))
...
VP <- computeVariancePartitioning(m, group = c(1,1,2,3), groupnames = c("urban", "nature", "temp"))

# plot var part
png(paste0(Output_wd, "/carabids_default_VP.png"), 
    width = 800)
...
Output_wd <- file.path(WorkDir, "5_Results/InvertebrateModels_Carabids_poisson/Predictions")


m$XFormula
# ~imperv.100m + dist.water.100m + temp.100m
...
m$Y
write.csv(predY2, paste0(Output_wd, "/carabids_predictions_pois_birdtransects.csv"))
...
grid.100mres.carabids <- grid.topredict.100mres %>%
  dplyr::select(x = x_100m, y = y_100m, 
                imperv.100m = impervious_surface_100m,
                dist.water.100m = distance_water_100m,
                temp.100m = temp_day_100m)
head(grid.100mres.carabids)


# Remove NAs to improve code efficiency
grid2.100mres <- grid.100mres.carabids[complete.cases(grid.100mres.carabids), ]
head(grid2.100mres)
str(grid2.100mres)
...
carabids.pred.abu <- rowSums(res.df[,-1], na.rm=TRUE)
# Richness per site. We decide to use an abundance higher than 0.5 as presence
carabids.pred.rich <- rowSums(res.df[,-1] > 0.5, na.rm=TRUE)
# Add coordinates and put all the data together
carabids.pred <- cbind(x = grid2.100mres$x, y = grid2.100mres$y, res.df, 
                      carabids.pred.abu = carabids.pred.abu, 
                      carabids.pred.rich = carabids.pred.rich)
# Save data
write.csv(carabids.pred , paste0(Output_wd, "/Predict_Berlin_100m_carabids_3sites.csv"), 
          row.names = F)
