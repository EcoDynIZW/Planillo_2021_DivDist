...
## Prediction map for grasshoppers1 
## Used a grid of 500m
...
filename = file.path(Model_wd, "model_pois_grasshop1_abu_3sites.Rdata")
load(file = filename)
...
m$XFormula
# ~imperv.100m + noise.100m + temp.500m + open.green.500m
...
grid.tmp2 <- read.csv(file.path(Data_wd, "/predict_grid_bibs_plots_cs_500m.csv"))
...
grid <- as.data.frame(cbind(x = grid.tmp1$x,
                            ...
                            temp.500m = grid.tmp2$temp_day,
                            open.green.500m = grid.tmp2$open_green))
...
# save results
write.csv(res.df , paste0(Output_wd, "/Predict_fullmap_grasshop1.csv"), row.names = F)
