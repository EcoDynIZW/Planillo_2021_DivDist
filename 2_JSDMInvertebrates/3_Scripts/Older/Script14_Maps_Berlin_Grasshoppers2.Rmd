---
title: "Maps_Berlin_Grasshoppers2"
author: "A.Planillo"
...
# Distribution maps of grasshoppers in Berlin

The maps area the prediction of the bees total abundance and bees species richness in the City of Berlin, based on a HMSC model built with observations in 42 plots, stored with the name of *grasshoppers2* (Joint Species Distribution Model Approach). These are the dry-grassland plots used in BIBS project for which there are data for all invertebrate groups (bees, carabids, grasshoppers, spiders). Species that only appeared in one or two plots were removed because they affect model convergence and are not very relevant to the whole community.

**The original model includes:**
* 18 species of grasshopppers (*rare species removed*)
* 42 plots (*shared by all the groups*)
* Environmental covariates: imperv.100m + open.green.500m (*standardized: centered and scaled*)
...
Load the model
```{r}
load(file= file.path(Model_wd, "model_pois_grasshop2_abu_3sites.Rdata"))
```
...
1.Divide the data into groups of 100 rows so the prediction will use less memory, doing one group at a time
```{r}
grid2$split <- dismo::kfold( x = grid2, k = floor(nrow(grid2)/100)+1 )
...
print(paste0("Number of folds = ", length(grid2.list), " (Groups of 100 rows)"))
head(grid2.list[[1]])
```
...
res.list <- parLapply(cl, grid2.list, my.predict.fun) # function to run the code in parallel and object to store the results
...
# Get general abundance and richness
grasshop.abu <- rowSums(res.df[,-1])# Abundance per site. We remove the first columns because it indicates the number of the line
grasshop.rich <- rowSums(res.df[,-1] > 0.5)# Richness per site. We decide to use an abundance higher than 0.5 as presence

# Add coordinates 
grasshop.pred <- cbind(x=grid2$x, y=grid2$y, res.df, abundance=grasshop.abu, richness=grasshop.rich)
head(grasshop.pred)

# save results
write.csv(grasshop.pred , paste0(Output_wd, "/Predict_grasshoppers_Berlin.csv"), row.names = F)
```
...
```{r, eval=FALSE}
grasshop.pred <- read.csv(paste0(Output_wd, "/Predict_grasshoppers_Berlin.csv"))
head(grasshop.pred)
str(grasshop.pred)
```
...
raster.grasshop.abu <- setValues(raster.500m, grasshop.pred$abundance)
raster.grasshop.rich <- setValues(raster.500m, grasshop.pred$richness)

rasters.grasshop <- stack(raster.grasshop.abu, raster.grasshop.rich)
names(rasters.grasshop) <- c("grasshop.abund", "grasshop.rich")
```
...
```{r, echo=FALSE}
map.abu.berlin <- tm_shape(rasters.grasshop.mask) +
  tm_raster("grasshop.abund", title = "Predicted grasshopper abundance",
...
tm_layout(main.title = "Grasshopper abundance \nres = 500m",
            compass.type = "4star",
...
map.abu.berlin
tmap_save(map.abu.berlin, filename = paste0(Output_wd, "/Map_berlin_grasshoppers_abu.png"), 
          dpi = 600)
...
```{r, echo=FALSE}
map.rich.berlin <- tm_shape(rasters.grasshop.mask) +
  tm_raster("grasshop.rich", title = "Predicted grasshopper species richness",
             # palette = rev(heat.colors(10)),
             palette = "Greens",
             style = "cont") +
tm_shape(water) +
...
tm_shape(berlin.out) +
  tm_borders("ivory4", lwd = 2.5) +
tm_layout(main.title = "Grasshopper species richness \nres = 500m",
            compass.type = "4star",
            legend.title.size = 1.2,
            legend.text.size = 0.8, 
          legend.bg.color = "white",
...
map.rich.berlin
tmap_save(map.rich.berlin, filename = paste0(Output_wd, "/Map_berlin_grasshoppers_richness.png"), 
          dpi = 600)
```
