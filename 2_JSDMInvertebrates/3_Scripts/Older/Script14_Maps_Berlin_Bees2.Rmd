...
# Distribution maps of wild bees in Berlin

The maps area the prediction of the bees total abundance and bees species richness in the City of Berlin, based on a HMSC model built with observations in 42 plots, stored with the name of *bees2* (Joint Species Distribution Model Approach). These are the dry-grassland plots used in BIBS project for which there are data for all invertebrate groups (bees, carabids, grasshoppers, spiders). Species that only appeared in one or two plots were removed because they affect model convergence and are not very relevant to the whole community.

**The original model includes:**
* 38 species of wild bees (*rare species removed*)
* 42 plots (*shared by all the groups*)
* Environmental covariates: imperv.500m + open.green.500m (*standardized: centered and scaled*)
...
rm(list = ls())
getMKLthreads()
setMKLthreads(4) #using many cores
```
...
Load the model
```{r}
load(file= file.path(Model_wd, "model_pois_bees2_abu_3sites.Rdata"))
```
...
grid.tmp500m <- read.csv(file.path(Data_wd, "/predict_grid_bibs_plots_cs_500m.csv"))
grid <- as.data.frame(cbind(x = grid.tmp500m$x,
                            y = grid.tmp500m$y,
                            imperv.500m = grid.tmp500m$impervious_surface,
                            open.green.500m = grid.tmp500m$open_green))
...
1.Divide the data into groups of 100 rows so the prediction will use less memory 
```{r}
# doing each group at a time. Assigning each row to a different groups randomly
...
print(paste0("Number of folds = ", length(grid.list), " (Groups of 100 rows)"))
head(grid.list[[1]])
```
...
clusterExport(cl, c("m")) # export objects to use to all cores
clusterEvalQ(cl, library("Hmsc")) # export packages to use to all cores
res.list <- parLapply(cl, grid.list, my.predict.fun) # function to run the code in parallel and object to store the results
...
# Re-order so it goes back to the original order
...
# add general results of abundance and richness
bees.abu <- rowSums(res.df[,-1])# Abundance per site. We remove the first columns because it indicates the number of the line
bees.rich <- rowSums(res.df[,-1] > 0.5)# Richness per site. We decide to use an abundance higher than 0.5 as presence

# Add coordinates and save results
bees.pred <- cbind(x=grid$x, y=grid$y, res.df, abundance=bees.abu, richness=bees.rich)
head(bees.pred)

# Save results
write.csv(bees.pred , paste0(Output_wd, "/Predict_bees_Berlin.csv"), row.names = F)
```


## Second step: plot the predicted map

If we start at this poinst, we should load the data file
```{r, eval=FALSE}
bees.pred <- read.csv(paste0(Output_wd, "/Predict_bees_Berlin.csv"))
...
raster.bees.abu <- setValues(raster.500m, bees.pred$abundance)
raster.bees.rich <- setValues(raster.500m, bees.pred$richness)

rasters.bees <- stack(raster.bees.abu, raster.bees.rich)
names(rasters.bees) <- c("bees.abund", "bees.richness")

writeRaster(raster.bees.abu, paste0(Output_wd, "/raster_bees_pred_abund_25833.tif"))
writeRaster(raster.bees.rich, paste0(Output_wd, "/raster_bees_pred_richness_25833.tif"))
```

We also crop the raster to berlin city
```{r}
...
```{r, echo=FALSE}
map.abu.berlin <- tm_shape(rasters.bees.mask) +
...
map.abu.berlin
tmap_save(map.abu.berlin, filename = paste0(Output_wd, "/Map_berlin_bees_abu.png"), 
          dpi = 600)
```
...
```{r, echo=FALSE}
map.rich.berlin <- tm_shape(rasters.bees.mask) +
  tm_raster("bees.richness", title = "Wild bees species richness",
             # palette = rev(heat.colors(10)),
             palette = "Greens",
             style = "cont") +
tm_shape(water) +
...
tm_shape(berlin.out) +
  tm_borders("ivory4", lwd = 2.5) +
# tm_shape(difference.spatial) +
#     tm_symbols(col="observed.rich", alpha = 1, size = 0.4,
#                border.col = "black",
#                palette = "Greens", 
#                legend.col.show = FALSE) +
tm_layout(main.title = "Wild bees species richness \nres = 500m",
            compass.type = "4star",
            legend.title.size = 1.2,
            legend.text.size = 0.8, 
          legend.bg.color = "white",
...
tmap_save(map.rich.berlin, filename = paste0(Output_wd, "/Map_berlin_bees_richness.png"), 
          dpi = 600)
```
