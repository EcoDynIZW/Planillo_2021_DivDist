...
# CREATING MAPS of RESIDUALS (OBSERVED - PREDICTED VALUES) 
...
# model.names <-  c("spiders1", "spiders2")
species <- c("bees", "bees",
             "carabids", "carabids",
             "grasshoppers", "grasshoppers", "grasshoppers", "grasshoppers")
# species <- c("spiders", "spiders")

for( i in 1:length(model.names)){
  obs.sp <- species[i]
  modelType <- model.names[i]
  observed.sp <- read.csv(paste0(output_wd, "/", modelType, '/Observed_abu_richness_', 
                                 obs.sp, '.csv'))
  predicted.sp <- read.csv(paste0(output_wd, "/", modelType, '/Predictions_bibsplots_', 
                                  modelType, '.csv'))
  Abu.dif <- predicted.sp$Abu.pred - observed.sp$observed.abu
  Rich.dif <- predicted.sp$S.pred - observed.sp$observed.s

  difference <- cbind.data.frame(observed.sp[,1:3], Abu.dif, Rich.dif)

  # Make them spatial object
  difference.spatial <- st_as_sf(difference, coords = c("X", "Y"), crs = 25833)

  # Make and save map for abundance
  default.breaks <- c(round(min(difference$Abu.dif), 0), round(quantile(difference$Abu.dif)[2:4], 0), round(max(difference$Abu.dif), 0))
  default.title <- paste0(modelType, ". \nResiduals in predicted abundance in BIBS plots (Pred - obs)")
  output <- paste0(output_wd, "/", modelType, '/Map_residuals_bibsplots_Abu_', 
                   obs.sp, '.pdf')

  see_save_pred_map(prediction.file = difference.spatial, bubbles.size = 1, bubbles.col = "Abu.dif",  
                    breaks = default.breaks, main.title = default.title, 
                    print.map = TRUE, save.mymap = TRUE, output = output,
                    my.color.scale = "PuOr")

  # Make and save map for richness
  default.breaks <- c(round(min(difference$Rich.dif), 0), round(quantile(difference$Rich.dif)[2:4], 0), round(max(difference$Rich.dif), 0))
  default.title <- paste0(modelType, ". \nResiduals in predicted richness in BIBS plots (Pred - obs)")
  output <- paste0(output_wd, "/", modelType, '/Map_residuals_bibsplots_Rich_', 
                   obs.sp, '.pdf')
  see_save_pred_map(prediction.file = difference.spatial, bubbles.size = 1, bubbles.col = "Rich.dif",  
                    breaks = default.breaks, main.title = default.title, 
                    print.map = TRUE, save.mymap = TRUE, output = output,
                    my.color.scale = "Spectral")
}
...
for (i in 1:length(species)){
  ...
  write.csv(observations, file=paste0(output_wd, "/", modelType, '/Observations_bibsplots_', 
