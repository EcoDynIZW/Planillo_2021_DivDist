...
# Distribution maps of carabids in Berlin

The maps area the prediction of the bees total abundance and bees species richness in the City of Berlin, based on a HMSC model built with observations in 42 plots, stored with the name of *carabids2* (Joint Species Distribution Model Approach). These are the dry-grassland plots used in BIBS project for which there are data for all invertebrate groups (bees, carabids, grasshoppers, spiders). Species that only appeared in one or two plots were removed because they affect model convergence and are not very relevant to the whole community.

**The original model includes:**
* 73 species of wild bees (*rare species removed*)
* 42 plots (*shared by all the groups*)
* Environmental covariates: imperv.100m + dist.water.100m + temp.100m     (*standardized: centered and scaled*)
...
Load the model
```{r}
load(file= file.path(Model_wd, "model_pois_carabids2_abu_3sites.Rdata"))
```
...
grid <- as.data.frame(cbind(x = grid.tmp100m$x,
                            ...
                            dist.water.100m = grid.tmp100m$distance_water,
...
# Paste all results in the same dataframe and recover the original order of the raster cells, 
# so we can paste it in a raster to create a new map
...
carabids.abu <- rowSums(res.df[,-1])# Abundance per site. We remove the first columns because it indicates the number of the line
carabids.rich <- rowSums(res.df[,-1] > 0.5)# Richness per site. We decide to use an abundance higher than 0.5 as presence

# Add coordinates 
carabids.pred <- cbind(x=grid2$x, y=grid2$y, res.df, abundance=carabids.abu, richness=carabids.rich)
head(carabids.pred)

# save results
write.csv(carabids.pred , paste0(Output_wd, "/Predict_carabids_Berlin.csv"), row.names = F)
```
...
```{r, eval=FALSE}
carabids.pred <- read.csv(paste0(Output_wd, "/Predict_carabids_Berlin.csv"))
head(carabids.pred)
str(carabids.pred)
```
...
cells <- cellFromXY(raster.500m, as.matrix(carabids.pred[, c("x", "y")]))

raster.abu <- raster.500m
raster.rich <- raster.500m

# Assign values to raster cells
raster.abu[cells] <- carabids.pred$abundance
plot(raster.abu)

raster.rich[cells] <- carabids.pred$richness
plot(raster.rich)

rasters.carabids <- stack(raster.abu, raster.rich)
...
```{r, echo=FALSE}
map.abu.berlin <- tm_shape(rasters.carabids.mask) +
  tm_raster("carabids.abund", title = "Predicted carabids abundance",
...
tm_layout(main.title = "Carabids abundance \nres = 500m",
            compass.type = "4star",
...
map.abu.berlin
tmap_save(map.abu.berlin, filename = paste0(Output_wd, "/Map_berlin_carabids_abu.png"), 
          dpi = 600)
...
```{r, echo=FALSE}
map.rich.berlin <- tm_shape(rasters.carabids.mask) +
  tm_raster("carabids.richness", title = "Predicted carabids species richness",
             # palette = rev(heat.colors(10)),
             palette = "Greens",
             style = "cont") +
tm_shape(water) +
...
tm_shape(berlin.out) +
  tm_borders("ivory4", lwd = 2.5) +
tm_layout(main.title = "Carabids species richness \nres = 500m",
            compass.type = "4star",
            legend.title.size = 1.2,
            legend.text.size = 0.8, 
          legend.bg.color = "white",
...
map.rich.berlin
tmap_save(map.rich.berlin, filename = paste0(Output_wd, "/Map_berlin_carabids_richness.png"), 
          dpi = 600)
```
